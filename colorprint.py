...
def log(F):
    ...
    def wrapper(*args, **kwargs):
        ...
        FORMAT = "{time} - {level: <8} - {name} {function} [{line}]: ".format(time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())),
                                            level=F.__name__.upper(),line = sys._getframe().f_back.f_lineno,name = (sys._getframe(1).f_code.co_filename).split('/')[-1],function = sys._getframe(1).f_code.co_name)
